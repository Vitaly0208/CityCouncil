...
const ProfilePage = () => {
    ...
    const userRank = useMemo(() => {
        const score = profile?.ratingScore;

        if (!profile || score === undefined || score === null || score <= 0) {
            return "-";
        }

        const activeUsers = leaderboard.filter(u => (u.ratingScore) > 0);
        ...
        activeUsers.forEach(u => {
            const userScore = u.ratingScore;
            if (!groups[userScore]) groups[userScore] = [];
            groups[userScore].push(u);
        });
        ...
        for (const scoreVal of sortedScores) {
            const tiedUsers = groups[scoreVal];
            const shuffled = [...tiedUsers].sort(() => Math.random() - 0.5);

            for (const user of shuffled) {
                if (String(user.userId ?? user.UserId) === String(profile.id)) {
                    return currentRank;
                }
                currentRank++;
            }
    ...
    const sessionAttendance = profile?.attendances  || [];
    const acceptedInitiatives = profile?.acceptedInitiatives || [];
                ...
                <aside className={styles.profileCard}>
                    ...
                    <div className={styles.contactList}>
                        ...
                        <span className={styles.ratingBadge}>
                            <span>Место в рейтинге: <strong>{userRank}</strong></span>
                        </span>
                        </div>
                    </div>
                </aside>
